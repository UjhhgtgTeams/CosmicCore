...
public static class Certificate
{
    public static X509Certificate GetOrGenX509Cert(string path, string password)
    {
        if (!Path.Exists(path))
        {
            return GenX509Cert(path, password);
        }
        else
        {
            try
            {
                return new X509Certificate(path, password);
            }
            catch (CryptographicException)
            {
                Log.Error("Certificate password incorrect! Using a temporary newly-generated certificate...");
                return GenX509Cert(path, password);
            }
        }
    }
    ...
    private static X509Certificate GenX509Cert(string path, string password)
    {
        ...
        certGen.AddExtension(
            ...
            false,
            new ExtendedKeyUsage(new[] { new DerObjectIdentifier("1.3.6.1.5.5.7.3.1") }));
        ...
        var newCert = certGen.Generate(new Asn1SignatureFactory("MD5WithRSA", keyPair.Private));
        var rawCert = DotNetUtilities.ToX509Certificate(newCert).Export(X509ContentType.Cert, password);
        File.WriteAllBytes(path, rawCert);
    ...
    private static X509Certificate GetX509CertWithPrivKey(string path, string password)
    {
        ...
        var rawPubCert = pubCert.Export(X509ContentType.Cert);
        var rawPrivCert = privCert.Export(X509ContentType.Pfx);
        var combinedCert = new X509Certificate2(rawPubCert, "", X509KeyStorageFlags.Exportable);

        using var privateKey = privCert.GetRSAPrivateKey();
        var rsaProvider = combinedCert.GetRSAPrivateKey();
        rsaProvider.ImportRSAPrivateKey(privateKey.ExportRSAPrivateKey(), out _);
